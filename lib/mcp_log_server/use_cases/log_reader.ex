defmodule McpLogServer.UseCases.LogReader do
  @moduledoc """
  Delegation facade — routes to the focused use-case modules.

  Retained for backward compatibility with callers of the historical
  `LogReader` API; new code should call the use-case modules directly.
  """
  ...
  @doc "List all logs in the given directory."
  @spec list_files(String.t()) :: {:ok, [file_info()]}
  def list_files(log_dir), do: UseCases.ListLogs.run(log_dir)
  ...
  def search(log_dir, file, pattern, opts \\ []),
    do: UseCases.SearchLogs.run(log_dir, file, pattern, opts)
  ...
  def get_stats(log_dir, file), do: UseCases.CollectStats.run(log_dir, file)

  @doc "Return the time range of a log file."
  @spec time_range(String.t(), String.t()) :: {:ok, map()} | {:error, String.t()}
  def time_range(log_dir, file), do: UseCases.TimeRange.run(log_dir, file)
end
